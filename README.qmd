...
Last successfull data update: `r paste0(format(Sys.time(), tz="America/Chicago"), " CST")`

# Product Link

[Access work-in-progress here](https://kexite.shinyapps.io/soccer_dashboard/)
...
2. Europa League and Conference League are not in the data, while UCL is... need to fix this issue, these are some of the best competiitons 
to scout players 
...
I plan to set up github automation to collect data every Tuesday, once the weekend games are finished. 
...
1. Identify top 10-15 attributes, or actions, a player does best (we can give user an ability to do so interactively). 
  ...
  there are 6 other metrics, or statistics, or numbers for actions, that are in 91st, or higher quartiles across all recorded
  players. 
...
3. Instead of selecting top features we can give a user an option to select features by hand from a list of 100 metrics. 
...
My biggest programming challenge of scraping the data is solved. Further challenges will arise if the HTML structure of the 
website changes, and I need to redesign some web scraping functions. Hopefully FBref do not indeed to do so in the next few 
months. 
